...
def split(path,nr_thread,has_header):
    ...
    def calc_nr_lines_per_thread():
        nr_lines = int(list(subprocess.Popen('wc -l {0}'.format(path),shell=True, stdout=subprocess.PIPE).stdout)[0].split()[0])
        if not has_header:
            nr_line += 1
...
def parallel_convert(cvt_path, arg_paths, nr_thread):
    ...
    for i in range(nr_thread):
        cmd = '{0}'.format(os.path.join('.',cvt_path)) #当前目录下的2.py命令
